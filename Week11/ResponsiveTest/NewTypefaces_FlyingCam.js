...
let tSize = 50; // text size of sidewords.
let poseNet, pose, video, phrase, phrase2, titlesize, titleY;
let marginX = 40;
let videosize;
let camx = 0;
...
function draw(){
  ...
  videosize = map(width, 0,1800,200,640);
  
  // Title
  push();
  textAlign(LEFT);
  textSize(titlesize);
  titlesize = map(width,0,1800,0,320);
  fill(0);
  textFont(font1);
  ...
  console.log(poses); // write all poses in console log
    if (poses.length > 0) { // if a pose is found
      ...
      if (pose.nose.x > video.width/2) { // If you go LEFT
        newObjects.push(new Sidewords(marginX,wordcounter*tSize+420,phrase,tSize,LEFT)); 
        ...
        newObjects.push(new Sidewords(width-marginX,wordcounter2*tSize+420,phrase2,tSize,RIGHT));
