...
let titlesize, titleY, tSize; // responsive text variables
let marginX = 40;
let videosize;
let camx = 0;
  ...
  console.log(poses); // write all poses in console log
    if (poses.length > 0) { // if a pose is found
      ...
      if (pose.nose.x > video.width/2) { // If you go LEFT
        newObjects.push(new Sidewords(marginX,(wordcounter*tSize)+(titleY*1.8),phrase,tSize,LEFT)); 
        ...
        newObjects.push(new Sidewords(width-marginX,(wordcounter2*tSize)+(titleY*1.8),phrase2,tSize,RIGHT));
